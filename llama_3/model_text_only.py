...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        # Flag to determine if this layer should capture activations
        self.capture_activation = False
        self.residual_activations = None

        # Optional sparse autoencoder forward function to apply to the residual activation
        self.sae_forward_fn = None
    ...
    def forward(
        ...
        if self.capture_activation:
            self.residual_activations = x_normalized.detach().clone()
        # Apply sparse autoencoder forward function if provided
...
class Transformer(nn.Module):
    def __init__(
        ...
        # cast store_layer_activ and sae_layer_forward_fn to empty iterables if None
        store_layer_activ = store_layer_activ or []
        sae_layer_forward_fn = sae_layer_forward_fn or {}
        ...
        for layer_id in range(params.n_layers):
            ...
            if layer_id in store_layer_activ:
                layer.capture_activation = True
            if layer_id in sae_layer_forward_fn:
                layer.sae_forward_fn = sae_layer_forward_fn[layer_id]
        ...
        self.store_layer_activ = store_layer_activ
    ...
    @torch.inference_mode()
    def forward_with_activation_override(
        self,
        tokens: torch.Tensor,
        start_pos: int,
        override_layer: int,
